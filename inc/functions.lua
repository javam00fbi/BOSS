...
--================================{{  This Code For Lua UTF8 Support in lua }}===================================
...
function UTF8_chars(s, no_subs)
return coroutine.wrap(function () 
return UTF8_map(s, coroutine.yield, no_subs) end)
end
...
function UTF8_replace(s, map)
return s:gsub(pattern, map)
end
 
function UTF8_Strip(s)
...
if key == 1 then
json_data =  json_data ..'"'..GroupS..'":{"Title":"'..(redis:get(boss..'group:name'..GroupS):gsub('"','') or '')..'"'
else
json_data =  json_data..',"'..GroupS..'":{"Title":"'..(redis:get(boss..'group:name'..GroupS):gsub('"','') or '')..'"'
end
...
elseif redis:sismember(boss..':SUDO_BOT:'..ChatID,UserID) then 
